...
export const prebuild = async (buildDir: string) => {
  const appRoot = path.join(buildDir, "app");
  const routesDir = path.join(appRoot, "routes");

  const projectData = fs.readFileSync(
    path.join(buildDir, PORJECT_CONFIG_FILE),
    "utf-8",
  );
  ...
  const generatedDir = path.join(appRoot, "__generated__");

  await deleteFolderIfExists(generatedDir);
  fs.mkdirSync(generatedDir, { recursive: true });

  const templateContent = fs.readFileSync(
    path.join(buildDir, `template.tsx`),
    "utf8",
  );
